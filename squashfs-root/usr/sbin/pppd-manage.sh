...
stop_other_manage()
{
	manage_log "all manage $(pidof pppd-manage.sh) me $$"
	for pid in $(pidof pppd-manage.sh)
	do
		if [ $pid -ne $$ ];then
			kill $pid
		fi
	done
	manage_log "after clean $(pidof pppd-manage.sh) me $$"
}
